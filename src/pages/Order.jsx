...
export const Order = () => {
  ...
  // form states
  const [Email_f, setEmail_f] = useState(email);
  ...
  const [Userselection, setUserselection] = useState(
    "You haven't selected any carrier yet"
  );
  ...
  const handleQuotations = async (e) => {
    ...
    var distance = Math.floor(Math.abs(Zip_f - Zip_t) / 1000);
    var Fed_cost = parseInt(distance * (distance * 2));
    var Ups_cost = parseInt(distance * (distance * 3));
    var Usps_cost = parseInt(distance * (distance * 5));
    ...
    setUsps(Usps_cost1);
    setAddress_f(
      Street_f.concat(", ", Apt_f, ", ", City_f, ", ", State_f, ", ", Zip_f)
    );
    setAddress_t(
      Street_t.concat(", ", Apt_t, ", ", City_t, ", ", State_t, ", ", Zip_t)
    );
    if (Priority > 1) {
  ...
  const createOrder = (data, actions) => {
    ...
    return actions.order
      .create({
        purchase_units: [
          {
            description: "Enroute Shipping Option",
  ...
  const onApprove = (data, actions) => {
    return actions.order.capture().then(function (details) {
      ...
      paymentupdate();
      toast.success(
        "Order has been successfully placed. Please check email for details"
      );
    });
  };
        ...
        <div className="container2 mt-5 mb-5 col-10 col-sm-8 col-md-6 col-lg-3">
          ...
          <div id="Paypal" style={{ visibility: "hidden" }}>
            <PayPalScriptProvider
              options={{
                "client-id":
                  "Aa6eZBm_bIDj3VxPQjP6YtJxK8ngMscbX_OpeIUIDkSLbOss0Yd9MLjRPMN_kR_W0-LBc8H4NZE0fz-f",
              }}
            >
              <div>
                {show ? (
